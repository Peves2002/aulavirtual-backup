...
const filePath = path.join(__dirname, 'src', 'app', '(web)', 'blog', '[slug]', 'page.tsx');
...
export default async function BlogDetailPage({ params }: { params: { slug: string } }) {
  ...
  const tagsIds = articulo.etiquetas.map(e => e.id);
  const catIds = articulo.categorias.map(c => c.id);
  ...
  const relacionados = await prisma.articulo.findMany({
    where: {
      ...
      id: { not: articulo.id },
      OR: [
        { etiquetas: { some: { id: { in: tagsIds } } } },
        { categorias: { some: { id: { in: catIds } } } }
      ]
    },
    take: 3,
    include: { categorias: true }
...
console.log('Blog detail page updated completely');
