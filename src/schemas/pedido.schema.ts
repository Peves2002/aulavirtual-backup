...
export const crearPedidoManualSchema = z
  .object({
    ...
    rutas_ids: z.array(z.string().uuid('ID de paquete inválido')).default([]),
  ...
  .refine(data => data.cursos_ids.length > 0 || data.rutas_ids.length > 0, {
    message: 'Selecciona al menos un programa o un paquete',
    path: ['cursos_ids']
  })
...
export const listarPedidosAgrupadosQuerySchema = z.object({
  page: z.coerce.number().int().positive().default(1),
  limit: z.coerce.number().int().positive().max(5000).default(10),
  nombre: z.string().trim().optional(),
  departamento: z.string().trim().optional(),
...
export type ListarPedidosAgrupadosQuery = z.infer<typeof listarPedidosAgrupadosQuerySchema>
