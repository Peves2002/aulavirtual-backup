...
export const crearCursoSchema = z.object({
  ...
  tipo: z.enum(['CURSO', 'DIPLOMADO', 'ESPECIALIZACION', 'SEMINARIO', 'TALLER']).default('CURSO'),
  ...
  precio_usd: z.coerce.number().min(0, 'El precio en USD no puede ser negativo').optional().nullable(),
  precio_falso_usd: z.coerce.number().min(0, 'El precio tachado en USD no puede ser negativo').optional().nullable(),
...
export const actualizarCursoSchema = z.object({
  ...
  categoria_id: z.string().uuid('ID de categoría inválido').optional().nullable(),
  tipo: z.enum(['CURSO', 'DIPLOMADO', 'ESPECIALIZACION', 'SEMINARIO', 'TALLER']).optional(),
