...
import {
  ...
  Divider, IconButton, Tooltip, Avatar, FormControlLabel
} from '@mui/material'
...
// ─── Method tab ───────────────────────────────────────────────────────────────
const MethodTab = ({ icon, label, selected, onClick, color = 'var(--mui-palette-primary-main)' }: {
  icon: string; label: string; selected: boolean; onClick: () => void; color?: string
}) => (
  <Box onClick={onClick} sx={{
    ...
    transition: 'all 0.2s', '&:hover': { borderColor: color, bgcolor: `color-mix(in srgb, ${color} 5%, white)` }
  }}>
...
// ─── Main component ───────────────────────────────────────────────────────────
const ProductoIAPaymentForm = ({ producto }: { producto: Producto }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'izipay' | 'culqi' | 'mercadopago' | 'manual'>('culqi')
  ...
  const [confirmedOrder, setConfirmedOrder] = useState<{ pedidoId: string; numeroPedido: number; total: number; productos: string[] } | null>(null)
  ...
  const displayTotal = producto.precio
  ...
  }, [isCulqiEnabled, isIzipayEnabled, isManualEnabled, paymentMethod])
  ...
  const handlePaymentSuccess = useCallback(() => {
    ...
    setTimeout(() => router.push('/estudiante/mis-gpts?success=1'), 2000)
  }, [router])
...
return true
  ...
  const handlePaymentResponse = useCallback(async (response: any, pedidoId: string) => {
    ...
    } catch { setPaymentError('Error inesperado al confirmar el pago') }
...
return }
 ...
 throw new Error(vd.message || 'Error al subir el comprobante') }
      ...
      if (whatsappNumero) {
        ...
        const productosFormateados = (titulosProductos as string[]).map((t: string) => `  • ${t}`).join('\n')
      ...
      setConfirmedOrder({ pedidoId, numeroPedido, total: Number(total), productos: titulosProductos as string[] })
      ...
      <Paper elevation={0} sx={{ p: { xs: 3, md: 5 }, borderRadius: '24px', bgcolor: 'white', border: '1px solid', borderColor: 'divider' }}>
        <Stack spacing={3} alignItems='center' sx={{ py: 4 }}>
          ...
          <Typography variant='body1' color='text.secondary' textAlign='center'>
            Tu acceso al GPT ha sido confirmado. Serás redirigido en unos segundos...
          </Typography>
        ...
        <Stack spacing={4}>
          ...
          <Box>
            ...
            <Typography variant='body2' color='text.secondary'>
              {isGuest ? 'Identifícate e ingresa tus datos para finalizar la compra.' : 'Verifica tus datos y completa el pago.'}
            </Typography>
          </Box>
          ...
          <Box>
            <Stack direction='row' alignItems='center' spacing={1} sx={{ mb: 2 }}>
              ...
              <Typography variant='subtitle1' fontWeight={700} color='text.primary'>Datos del Comprador</Typography>
            </Stack>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <TextField fullWidth label='Correo Electrónico' value={formData.correo}
            ...
            <Box>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={4}>
                  <TextField select fullWidth size='small' label='Tipo de Comprobante'
                    value={tipoComprobante} onChange={e => setTipoComprobante(e.target.value as any)}
          ...
          <Box>
            ...
            {(!isCulqiEnabled && !isIzipayEnabled && !isMercadoPagoEnabled && (!isManualEnabled || metodosManual.length === 0)) && (
            ...
            {paymentMethod === 'manual' && isManualEnabled && (
              <Box sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
                {isGuest ? (
                  <Box sx={{ p: 3 }}>
                    <Button variant='contained' fullWidth size='large' onClick={() => openLogin()}
...
return (
                            <Box key={m.id} onClick={() => setSelectedMetodoManualId(m.id)} sx={{
                              ...
                              bgcolor: isSelected ? 'primary.50' : 'white', '&:hover': { borderColor: 'primary.main' }
                            }}>
                              <Stack direction='row' alignItems='center' spacing={2} sx={{ p: 1.75 }}>
                                {m.imagen_url ? (
                                  <Avatar src={m.imagen_url} variant='rounded' sx={{ width: 44, height: 44, borderRadius: 1.5, border: '1px solid', borderColor: 'divider' }} />
                                ) : (
                                ...
                                <Box sx={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, border: '2px solid', borderColor: isSelected ? 'primary.main' : 'divider', bgcolor: isSelected ? 'primary.main' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                      ...
                      <Box sx={{ p: 3, bgcolor: 'grey.50' }}>
                        ...
                        <Box sx={{ bgcolor: 'white', borderRadius: 2, border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
                          ...
                          <Box sx={{ px: 2.5, py: 1.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'primary.50', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <Chip label={`${currencySymbol} ${displayTotal.toFixed(2)}`} color='primary' size='small' sx={{ fontWeight: 800, fontSize: '0.85rem' }} />
                          </Box>
                        </Box>
                    ...
                    <Box sx={{ p: 3 }}>
                      ...
                      <input ref={fileInputRef} type='file' accept='image/jpeg,image/png,image/webp' style={{ display: 'none' }} onChange={handleVoucherChange} />
                      {voucherPreview ? (
                        <Box sx={{ position: 'relative' }}>
                          ...
                          <Box sx={{ position: 'absolute', top: 8, right: 8 }}>
                            <IconButton size='small' onClick={() => { setVoucher(null); setVoucherPreview(null) }}
                        ...
                        <Box sx={{ border: '2px dashed', borderColor: 'divider', borderRadius: 2.5, p: 4, textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s', '&:hover': { borderColor: 'primary.main', bgcolor: 'primary.50' } }}
                          ...
                          <Typography variant='caption' color='text.disabled'>o arrastra tu imagen aquí</Typography>
                        </Box>
                      )}
                    </Box>
                    ...
                    <Box sx={{ p: 3, bgcolor: voucher ? 'primary.50' : 'grey.50' }}>
                      <Button variant='contained' fullWidth size='large' onClick={handleManualCheckout} disabled={isLoading || !voucher}
                      ...
                      <Typography variant='caption' color='text.secondary' textAlign='center' display='block' sx={{ mt: 1.5 }}>
                        Tu pedido quedará en revisión. El acceso al GPT se activa al verificar el pago.
                      </Typography>
                    </Box>
                  </Stack>
                )}
      ...
      <AppModal open={confirmModalOpen} handleClose={() => { setConfirmModalOpen(false); router.push('/estudiante/mis-gpts') }}
        ...
        <Box sx={{ bgcolor: 'success.main', px: 4, pt: 4, pb: 3, textAlign: 'center' }}>
          ...
          <Typography variant='h4' fontWeight={900} color='white' letterSpacing={0.3}>¡Pedido Registrado!</Typography>
          {confirmedOrder && (
        ...
        <Box sx={{ pt: 3, pb: 3, px: 3 }}>
          <Stack direction={{ xs: 'column', sm: whatsappQr ? 'row' : 'column' }} spacing={3} alignItems='flex-start'>
            <Box sx={{ flex: 1, minWidth: 0, width: '100%' }}>
              {confirmedOrder && (
                <Box sx={{ mb: 2.5, p: 2.5, borderRadius: 2, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider' }}>
                  ...
                  <Stack spacing={1}>
                    {confirmedOrder.productos.map((p, i) => (
                      <Stack key={i} direction='row' alignItems='flex-start' spacing={1.25}>
                        <Box sx={{ width: 28, height: 28, borderRadius: 1.5, bgcolor: 'success.lighterOpacity', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <i className='tabler-robot' style={{ fontSize: 15, color: '#25927F' }} />
                        </Box>
                        <Typography variant='body2' fontWeight={600} lineHeight={1.45} sx={{ pt: 0.4 }}>{p}</Typography>
                      </Stack>
                    ))}
                  </Stack>
                  <Divider sx={{ my: 1.5 }} />
                  <Stack direction='row' justifyContent='space-between' alignItems='center'>
                    ...
                    <Typography variant='h6' fontWeight={800} color='success.main'>{currencySymbol} {confirmedOrder.total.toFixed(2)}</Typography>
                  </Stack>
                </Box>
              )}
              ...
              {voucherPreview && (
                <Box sx={{ mb: 2.5 }}>
                  ...
                  <Box component='img' src={voucherPreview} alt='Comprobante'
                    sx={{ width: '100%', maxHeight: 180, objectFit: 'contain', borderRadius: 2, border: '1.5px solid', borderColor: 'divider', bgcolor: '#f8fafc', cursor: 'zoom-in' }}
                    onClick={() => window.open(voucherPreview!, '_blank')} />
                </Box>
              )}

              <Alert severity='info' icon={<i className='tabler-clock' style={{ fontSize: 18 }} />} sx={{ mb: 2.5, borderRadius: 2, fontSize: '0.8125rem' }}>
                Tu pedido está en revisión. El acceso al GPT se activa una vez verificado el pago.
              </Alert>
              ...
              <Stack spacing={1.5}>
                ...
                <Button fullWidth variant='outlined' size='large' startIcon={<i className='tabler-robot' />}
                  onClick={() => { setConfirmModalOpen(false); router.push('/estudiante/mis-gpts') }}
            ...
            {whatsappQr && (
              <Box sx={{ width: { xs: '100%', sm: 230 }, flexShrink: 0, textAlign: 'center', py: 3, px: 2, bgcolor: '#f0fdf4', borderRadius: 3, border: '1.5px dashed', borderColor: '#86efac', alignSelf: 'stretch', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1.5 }}>
                <i className='tabler-brand-whatsapp' style={{ fontSize: 30, color: '#25D366' }} />
                <Typography variant='subtitle2' fontWeight={700} sx={{ fontSize: '0.82rem' }}>Enviar por WhatsApp</Typography>
                <Typography variant='caption' color='text.secondary' sx={{ fontSize: '0.72rem', lineHeight: 1.5 }}>
                  Escanea desde tu celular — se abrirá WhatsApp con el resumen listo para enviar
                </Typography>
                ...
                <Typography variant='caption' color='text.disabled' sx={{ fontSize: '0.68rem' }}>Abre la cámara y apunta al QR</Typography>
              </Box>
            )}
          </Stack>
        </Box>
...
export default ProductoIAPaymentForm
