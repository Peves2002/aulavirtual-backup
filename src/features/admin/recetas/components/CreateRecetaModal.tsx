...
import { useCreateReceta } from '../hooks/useRecetas'
...
const emptyValues: CrearRecetaDto = {
  nombre: '',
  imagen: null,
  descripcion: null,
...
export const CreateRecetaModal = ({ open, handleClose, onSuccess }: Props) => {
  const { enqueueSnackbar } = useSnackbar()
  const createMutation = useCreateReceta()

  const handleSubmit = async (values: CrearRecetaDto, { setSubmitting, resetForm }: FormikHelpers<CrearRecetaDto>) => {
    try {
      ...
      enqueueSnackbar(error?.message || error?.error || 'Error al crear receta', { variant: 'error' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <AppModal open={open} handleClose={handleClose}>
      <Box sx={{ mb: 3, textAlign: 'center' }}>
        <Typography variant='h4' fontWeight={600}>Nueva Receta</Typography>
        <Typography variant='body2' color='text.secondary'>Completa los datos de la receta</Typography>
      </Box>

      <Formik initialValues={emptyValues} validationSchema={toFormikValidationSchema(crearRecetaSchema)} onSubmit={handleSubmit}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <CustomTextField fullWidth label='URL de Imagen (Opcional)' name='imagen' value={values.imagen ?? ''}
                  onChange={e => setFieldValue('imagen', e.target.value || null)}
                  onBlur={handleBlur}
                  ...
                  <Box key={gi} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, p: 2, mb: 2 }}>
                    ...
                    {grupo.items.map((item, ii) => (
                      <Box key={ii} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
                        <CustomTextField size='small' label='Insumo' value={item.insumo}
                          sx={{ flex: 2 }}
                          onChange={e => { const g = [...values.insumos]; g[gi].items[ii] = { ...g[gi].items[ii], insumo: e.target.value }; setFieldValue('insumos', g) }}
                        />
                        <CustomTextField size='small' label='Cantidad (kg)' value={item.cantidad}
                          sx={{ flex: 1 }}
              ...
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mt: 2 }}>
                  ...
                  <Button variant='contained' type='submit' disabled={isSubmitting} sx={{ px: 4 }} startIcon={<i className='tabler-plus' />}>
