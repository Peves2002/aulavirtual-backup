...
import NoticiasSettings from '../../configuracion/components/NoticiasSettings'

interface Props {
  initialData?: Configuracion[]
}

export function NoticiasClient({ initialData }: Props) {
...
return s?.user?.accessToken ?? null
      ...
      enqueueSnackbar('Noticias actualizadas correctamente.', { variant: 'success' })
    } catch (err) {
      console.error(err)
      enqueueSnackbar('Error al guardar las noticias', { variant: 'error' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Paper sx={{ p: 3, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h5" fontWeight={700}>Gestión de Noticias</Typography>
        <Button variant="contained" onClick={handleSave} disabled={saving} startIcon={<i className="tabler-device-floppy" />}>
          {saving ? 'Guardando...' : 'Guardar Cambios'}
        </Button>
      </Stack>
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        <NoticiasSettings config={config} onInputChange={handleInputChange} />
      </Box>
    </Paper>
  )
}
